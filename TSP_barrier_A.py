# -*- coding: utf-8 -*-
"""
Created on Thu May 23 15:20:00 2019

"""

"""TSP.py with barriers
"""
...
def runtest(mapfile, start, goal, verbose = True):
  ...
  # RP1 = RRT.RobotPlanner(boundary, blocks)  # create instance of RRT.RobotPlanner
  RP2 = A_star_2d.RobotPlanner(boundary, blocks)  # create instance of A_star.RobotPlanner
  ...
  last_stop = start
  i = 1
  newrobotpos, path = RP2.Astar_Planner(robotpos, goal)
  return path
...
class MyTSP(object):
        ...
        for i in range(self.n):
            ...
            while right_time == 0:
                ...
                for j in range(len(self.barrier)):
                    if self.barrier[j][0]-10 <= x <= self.barrier[j][2]+10 and self.barrier[j][1]-10 <= y <= self.barrier[j][3]+10:
                        error_time += 1
                if error_time == 0:
                    break
                    #coor.append([x,y])
                    #if i == 0:
                       # if (np.array(coor[i])-np.array(self.starter))[0]**2 + (np.array(coor[i])-np.array(self.starter))[1]**2 > 1600:
                       #break  
                    #else:
                        #if (np.array(coor[i])-np.array(coor[i-1]))[0]**2 + (np.array(coor[i])-np.array(coor[i]))[1]**2 > 1600:
                        #break
    ...
    def distance(self, order):
        ...
        for i in range(-1, self.n):
            ...
            #print(p1,p2,q1,q2)
            path = runtest('./2Dbuilding.txt', np.array(q1), np.array(q2), True)
    ...
    def line(self, order):
        ...
        for i in range(-1, self.n):
            ...
            for j in range(len(path)-1):
                p1, p2 = path_return[j], path_return[j+1]
                self.canvas.create_line(p1[0], p1[1], p2[0], p2[1], fill="#000000", tags="line")
